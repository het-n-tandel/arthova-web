            ...
            <tbody>
              {mfHoldings.map((mf: any) => (
                <tr key={mf.id} className="border-t border-border-default hover:bg-bg-surface-2 transition-colors h-[52px]">
                  ...
                  <td className="px-4 py-2 text-[13px] text-text-secondary" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINR(mf.quantity * mf.avgCost)}</td>
                  <td className="px-4 py-2 text-[13px] text-text-primary" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINR(mf.quantity * mf.cmp)}</td>
                  <td className="px-4 py-2"><DeltaBadge value={((mf.cmp - mf.avgCost) / mf.avgCost) * 100} /></td>
                </tr>
              ))}
              {mfHoldings.length === 0 && (
                <tr>
                    <td colSpan={4} className="text-center py-4 text-text-faint text-[13px]">No Mutual Funds found</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {activeTab === 'Gold & Silver' && (
        <div className="overflow-x-auto rounded-[12px] border border-border-default">
          <table className="w-full">
            ...
            <tbody>
              {goldHoldings.map((h: any) => (
                <tr key={h.id} className="border-t border-border-default hover:bg-bg-surface-2 transition-colors h-[52px]">
                  <td className="px-4 py-2 text-[13px] font-medium text-text-primary">{h.name || h.assetType}</td>
                  <td className="px-4 py-2 text-[13px]" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{h.quantity}</td>
                  <td className="px-4 py-2 text-[13px] text-text-secondary" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINR(h.quantity * h.avgCost)}</td>
                  <td className="px-4 py-2 text-[13px] text-text-primary" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINR(h.quantity * h.cmp)}</td>
                  <td className="px-4 py-2"><DeltaBadge value={((h.cmp - h.avgCost) / h.avgCost) * 100} /></td>
                </tr>
              ))}
              {goldHoldings.length === 0 && (
                <tr>
                    <td colSpan={5} className="text-center py-4 text-text-faint text-[13px]">No Gold/Silver found</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <tbody>
              {fdHoldings.map((fd: any) => (
                <tr key={fd.id} className="border-t border-border-default hover:bg-bg-surface-2 transition-colors h-[52px]">
                  ...
                  <td className="px-4 py-2 text-[13px]" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINR(fd.quantity * fd.avgCost)}</td>
                </tr>
              ))}
              {fdHoldings.length === 0 && (
                <tr>
                    <td colSpan={2} className="text-center py-4 text-text-faint text-[13px]">No Fixed Deposits found</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {propHoldings.map((prop: any) => (
            <div key={prop.id} className="bg-bg-surface border border-border-default rounded-[12px] p-5">
              ...
              <p className="text-[12px] text-text-faint mb-4">{prop.quantity} units</p>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-eyebrow">Acquisition</span>
                  <p className="text-[16px] text-text-secondary mt-1" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINRCompact(prop.quantity * prop.avgCost)}</p>
                </div>
                <div>
                  <span className="text-eyebrow">Current Value</span>
                  <p className="text-[16px] text-text-primary mt-1" style={{ fontFamily: 'IBM Plex Mono, monospace', fontVariantNumeric: 'tabular-nums' }}>{formatINRCompact(prop.quantity * prop.cmp)}</p>
                </div>
              </div>
              <div className="mt-3"><DeltaBadge value={((prop.cmp - prop.avgCost) / prop.avgCost) * 100} size="md" /></div>
            </div>
          ))}
          {propHoldings.length === 0 && (
            <div className="col-span-full text-center py-8 text-text-faint text-[13px] border border-border-default rounded-[12px] bg-bg-surface">
                No Properties found
            </div>
          )}
        </div>
