...
export function HoldingsTable({ data, className }: HoldingsTableProps) {
        ...
        <table className="w-full">
          ...
          <tbody>
            {table.getRowModel().rows.map((row) => (
              <tr
                key={row.id}
                onClick={() => router.push(`/dashboard/stocks/${row.original.symbol}`)}
                className="border-t border-border-default hover:bg-bg-surface-2 transition-colors duration-150 h-[52px] cursor-pointer"
              >
